...
from utils.result_publisher import send_result, send_history
...
def run(params, config, capture, detector, reid):
    ...
    if params.output_broker:
        for i in range(len(output_detections[0])):
            send_result(output_detections[0][i])
        for i in range(len(tracker.get_all_tracks_history()[0])):
            send_history(tracker.get_all_tracks_history()[0][i])
        log.info('Output JSON sent to broker')
...
def main():
    ...
    parser.add_argument("--output_broker", help="Optional. Send output JSON files to the broker", action='store_true')
