...
'''
def draw_detection_pose(frame, detections, exit_id_list, exit_pose_list, sk_model, class_model, cfg, gif_cnt, img_cnt, show_all_detections=True):
    new_id_list = []
    new_pose_list = []
            ...
            if len(exit_id_list) > 0:
                for old_label in exit_id_list:
                    if old_label == id:
                        ...
                        # print("the loc is :  ")
                        # print(loc)
                        # print(len(exit_pose_list[loc]))
                        # print(exit_pose_list[loc])
        ...
        if id >= 0:
            ...
            if len(new_pose_list[i]) >= 16:
                ...
                if True:
                    ...
                    # print(down_list)
                    # print(id_0_walk)
                    # print("The img_cnt is :  ", end='')
                    # print(img_cnt)
                    #
                    # print("The old max_loc is :  ", end='')
                    # print(max_loc)
    ...
    return new_id_list, new_pose_list, gif_cnt, img_cnt


def visualize_multicam_detections(frames, all_objects, exit_id_list, exit_pose_list, sk_model, class_model, cfg, img_cnt, gif_cnt, fps='', show_all_detections=True,
    ...
    for i, (frame, objects) in enumerate(zip(frames, all_objects)):
        id_list, pose_list, gif_cnt, img_cnt = draw_detection_pose(frame, objects, exit_id_list, exit_pose_list, sk_model, class_model, cfg, gif_cnt, img_cnt, show_all_detections)
        if vis is not None:
            if stack_frames == 'vertical':
    ...
    return vis, id_list, pose_list, gif_cnt, img_cnt
