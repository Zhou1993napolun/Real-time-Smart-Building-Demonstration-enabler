        ...
        if detection.score > threshold:
            ...
            if dataset == 'df2':
                ...
                cv.putText(frame, '{} {:.1%}'.format(class_name, detection.score),
                            (xmin, ymin - 7), cv.FONT_HERSHEY_COMPLEX, 2, (0, 255, 255), 2)
...
'''
def draw_detection_pose(frame, detections, exit_id_list, exit_pose_list, sk_model, class_model, clo_model, detector_pipeline, cfg, gif_cnt, img_cnt, next_frame_id_to_show, next_frame_id, id_dress_list, id_dress_list_cnt, id_object_list, self_cnt, show_all_detections=True):
    new_id_list = []
    new_pose_list = []
    ...
    for i, obj in enumerate(detections):
        ...
        # 后面就是原始的画图程序了
        # this cover
        if show_all_detections or id >= 0 and False:
            cv.rectangle(frame, (left, top), (right, bottom), box_color, thickness=3)
        ...
        if id >= 0:
            ...
            if False:
                self_img = np.zeros((240, 320))
                for j in range(len(SKELETON)):
                    kpt_a, kpt_b = SKELETON[j][0], SKELETON[j][1]
                    x_a, y_a = new_pose_list[i][len(new_pose_list[i]) - 1][kpt_a][0], new_pose_list[i][len(new_pose_list[i]) - 1][kpt_a][1]
                    x_b, y_b = new_pose_list[i][len(new_pose_list[i]) - 1][kpt_b][0], new_pose_list[i][len(new_pose_list[i]) - 1][kpt_b][1]
                    cv.line(self_img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), 255, 2)
                cv.imwrite('/home/ubuntu/hws/reid_pose/self_work_img/{}.png'.format(self_cnt), self_img)
                self_cnt += 1
            ...
            if len(new_pose_list[i]) >= 16:
                ...
                for temp_i in range(16):
                    ...
                    # 中间生成骨架动图可视化
                    # cv.imwrite('/home/ubuntu/hws/reid_pose/self_work/{}.png'.format(self_cnt), mid_img)
    ...
    return new_id_list, new_pose_list, gif_cnt, img_cnt, next_frame_id_to_show, next_frame_id, id_dress_list, id_dress_list_cnt, id_object_list, self_cnt


def visualize_multicam_detections(frames, all_objects, exit_id_list, exit_pose_list, sk_model, class_model,  clo_model, detector_pipeline, cfg, img_cnt, gif_cnt, next_frame_id_to_show, next_frame_id, id_dress_list, id_dress_list_cnt, id_object_list, self_cnt, fps='', show_all_detections=True,
    ...
    for i, (frame, objects) in enumerate(zip(frames, all_objects)):
        id_list, pose_list, gif_cnt, img_cnt, next_frame_id_to_show, next_frame_id, id_dress_list, id_dress_list_cnt, id_object_list, self_cnt = draw_detection_pose(frame, objects, exit_id_list, exit_pose_list, sk_model, class_model, clo_model, detector_pipeline, cfg, gif_cnt, img_cnt, next_frame_id_to_show, next_frame_id, id_dress_list, id_dress_list_cnt, id_object_list, self_cnt, show_all_detections)
        if vis is not None:
            if stack_frames == 'vertical':
    ...
    return vis, id_list, pose_list, gif_cnt, img_cnt, next_frame_id_to_show, next_frame_id, id_dress_list, id_dress_list_cnt, id_object_list, self_cnt
